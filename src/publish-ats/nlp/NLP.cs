...
/// <summary>
///     Provides Natural Language Processing (NLP) utilities for optimizing markdown content for ATS (Applicant Tracking
///     Systems).
/// </summary>
internal static class Nlp
{
    ...
    internal static string OptimizeForAts(string markdown)
    {
        ...
        var sanitizedMarkdown = SanitizeMarkdown(markdown);
        ...
        var patterns = new Dictionary<string, (string pattern, HashSet<string> category)>
        {
            {
                "Technologies",
                (@"\b(JavaScript|TypeScript|Python|Java|C#|\.NET|React|Angular|Node\.js|AWS|Azure|SQL|Docker|Kubernetes|PostgreSQL|MongoDB|Git|REST|GraphQL|CI/CD|Terraform|Kafka|Redis|Cassandra|NoSQL|DevOps|Go|R)\b",
                    technologies)
            },
            {
                "JobTitles",
                (@"\b(Software Engineer|Software Developer|Senior Engineer|Lead Engineer|Principal Engineer|Architect|Cloud Architect|Platform Architect|CTO|Technical Director|Tech Lead|Project Manager|Technical Lead|Head of Architecture)\b",
                    jobTitles)
            },
            {
                "Skills",
                (@"\b(Cloud Architecture|Microservices|API Design|System Design|Database Design|Scalability|Performance Optimization|Security|ETL|ELT|Data Processing|Data Migration|Mentorship|Technical Leadership|Serverless|Cloud Migration)\b",
                    skills)
            },
            {
                "Education",
                (@"\b(Bachelor|Master|PhD|MSc|BSc|MBA|Doctorate)\s(of|in)\s(Science|Arts|Engineering|Computer Science|Business|Information Technology)\b",
                    education)
            }
        };
        ...
        if (summary.Length > 0) enhancedMarkdown = summary + "\n\n" + enhancedMarkdown;
    ...
    private static string SanitizeMarkdown(string markdown)
    {
        ...
        var sanitized = Regex.Replace(markdown, @"\*\*(.*?)\*\*", "$1");
    ...
    private static void ExtractKeyTermsFromBulletPoints(string markdown, HashSet<string> skills,
        HashSet<string> technologies)
    {
        ...
        {
            var content = bulletPoint.Groups[1].Value;
    ...
    private static bool IsPartOfFormattedText(string markdown, int position)
    {
        ...
        var startCheck = Math.Max(0, position - 50);
        var endCheck = Math.Min(markdown.Length, position + 50);

        var context = markdown.Substring(startCheck, endCheck - startCheck);
